...
namespace cv
{
int RANSACUpdateNumIters_4pra( double p, double ep, int modelPoints, int maxIters )
{
    if( modelPoints <= 0 )
...
class RANSACPointSetRegistrator_4pra : public PointSetRegistrator
{
public:
    RANSACPointSetRegistrator_4pra(const Ptr<PointSetRegistrator::Callback>& _cb=Ptr<PointSetRegistrator::Callback>(),
    ...
    bool run(InputArray _m1, InputArray _m2, OutputArray _model, OutputArray _mask) const CV_OVERRIDE
    {
        ...
        for( iter = 0; iter < niters; iter++ )
        {
            ...
            for( i = 0; i < nmodels; i++ )
            {
                ...
                if( goodCount > MAX(maxGoodCount, modelPoints-1) )
                {
                    ...
                    niters = RANSACUpdateNumIters_4pra( confidence, (double)(count - goodCount)/count, modelPoints, niters );
                }
            }
        }
        // std::cerr << "4pra within iter " << iter << " " << niters << " " << (double)(count - maxGoodCount)/count << std::endl;
        ...
        auto end = std::chrono::system_clock::now();
        // std::cerr << "4pra time " << std::chrono::duration<double>(end - start).count() << std::endl;
        // std::cerr << "-----------" << std::endl;
        ...
        int method, double prob, double threshold, OutputArray _mask)
{
    ...
    {
        auto reg = RANSACPointSetRegistrator_4pra(
                makePtr<PC4PRAEstimatorCallback>(angle), 4, threshold, prob);
